...
def test_items_endpoint(zotero_db):
    app = create_app(zotero_db)
    with TestClient(app) as client:
        res = client.get("/api/zotero/items")
    assert res.status_code == 200
...
def test_items_by_collection_endpoint(zotero_db):
    app = create_app(zotero_db)
    with TestClient(app) as client:
        res = client.get("/api/zotero/items", params={"collection_id": 3})
    assert res.status_code == 200
...
def test_search_endpoint(zotero_db):
    app = create_app(zotero_db)
    with TestClient(app) as client:
        res = client.get("/api/zotero/search", params={"q": "honeypot"})
    assert res.status_code == 200
...
def test_item_detail_endpoint(zotero_db):
    app = create_app(zotero_db)
    with TestClient(app) as client:
        res = client.get("/api/zotero/items/1")
    assert res.status_code == 200
    body = res.json()
...
def test_item_no_pdf_endpoint(zotero_db):
    app = create_app(zotero_db)
    with TestClient(app) as client:
        res = client.get("/api/zotero/items/6")
    assert res.status_code == 200
    body = res.json()
    assert body["title"] == "Ghost Paper"
...
def test_item_not_found_endpoint(zotero_db):
    app = create_app(zotero_db)
    with TestClient(app) as client:
        res = client.get("/api/zotero/items/999")
